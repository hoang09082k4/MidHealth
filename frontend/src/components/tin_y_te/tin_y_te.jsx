...
export function MucTinYTeTrangChu({ onNavigate, onSelectSpecialty }) {
  ...
  return (
    <section className="health-home-magazine" id="news">
      ...
      <div className="health-home-band"><div className="health-home-inner"><div className="health-home-toolbar"><div className="health-home-topic-tabs" role="tablist" aria-label="Danh muc tin y te noi bat">{homeTabs.map((tab) => <button type="button" className={tab.category === activeCategory ? 'active' : ''} key={tab.label} onClick={() => setActiveCategory(tab.category)}>{tab.label}</button>)}</div><HealthSearchBar activeCategory={activeCategory} searchTerm={searchTerm} onSearchTermChange={setSearchTerm} onSubmit={submitSearch} /></div>{isLoading ? <p className="health-mag-state">{TEXT.loadingArticles}</p> : <div className="health-home-rail" aria-label="Danh sach bai viet tin y te">{currentArticles.slice(0, 8).map((article) => <ArticleCard article={article} key={article.slug || article.id} onOpen={(slug) => onNavigate?.({ name: 'detail', slug })} />)}</div>}</div></div>
      <ExpertTeam
        experts={experts}
